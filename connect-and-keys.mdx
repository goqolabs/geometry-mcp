...
> **Roll** in the portal so Zuplo issues a new key. The old key stays valid for **7 days** and cannot be deleted early (vendor behavior). If you need the old key killed immediately, open a **[GitHub issue](https://github.com/goqolabs/geometry-mcp/issues/new)** (use title prefix `[urgent] key revoke`) — do not paste the secret key in the issue body. Optional: `support@geometry.app` for the same urgent revoke request.

## Support (builders)

Product / technical questions → **[GitHub Issues](https://github.com/goqolabs/geometry-mcp/issues)** on the public docs repo. Real builders already have GitHub; this keeps support out of the spam / phishing inbox. Billing card issues stay in the Stripe portal.
