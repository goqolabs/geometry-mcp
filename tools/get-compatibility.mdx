...
description: Two birth dates → structured compatibility score (Tool 1).
---
...
Live payloads are large (~85 KB) because each side embeds a full cosmic block. Below is a real teaching excerpt — `birth_card_a` / `birth_card_b` are abbreviated; on the wire they match [`get_card_for_birthdate`](/tools/get-card-for-birthdate) shape.
