...
One birth date in → the full cosmic identity out: birth card, ruling planet, card class, plus layered systems for that date (tarot minor + **dual major birth cards** Greer + Tarot School, medicine, Lenormand, runes, ogham, geomancy, sidereal, moon phase, season, retrogrades, and more).
...
Wondering which numbers to build on? See the [Field glossary](/field-glossary) — it separates tradition numbers like `card_no` and `tarot_no` from internal catalog ids like `system_no`, and explains why `element_no` reads differently in the classical and periodic blocks.
