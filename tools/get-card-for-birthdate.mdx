...
geometry.app shows a **teaching excerpt** only. This page is a public docs sample (values from a live call; season anchors verified). The sample below is **teaching shape** — shorter than the wire. A full Claude paste for the same date includes the complete blocks (`medicine`, `lenormand`, full `astro_wheels`, full `retrogrades_active`, and the rest).
