...
**Full payload:** call this tool with a date in range — the live response **is** the complete JSON. There is no extra flag or `full_payload` command. Ask your agent to show the full tool result if the UI truncates it. Treat the sample below as teaching shape; nested fields and array lengths can be longer on the wire.
