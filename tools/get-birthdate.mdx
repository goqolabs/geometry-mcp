...
One birth date in → birth card and layered systems for that date: tarot (minor + **dual major** Greer and Tarot School), medicine, Lenormand, runes, ogham, geomancy, sidereal wheels, moon phase, season, retrogrades, **karmic source/spirit**, **14 planetary Life-spread cards**, and more.

<Info>
  **When to use:** any **single** Gregorian date for moon phase (incl. blue / blood / supermoon flags, Earth–Moon distance, prev/next blue or blood dates), season / equinox–solstice anchors, classical retrogrades (`retrogrades_active`), calendars, lunar year, birth card, **weekday / leap year** (`calendar_meta.weekday`, `is_leap_year` from `date_dim`), or full date identity. **Prefer this over** [`get_day`](/tools/get-day) for astronomy, calendar facts, and “what was the sky” questions. `get_day` is day card + weekday **ruler** only — it does **not** return `is_leap_year`, moon, season, or Rx.
</Info>
...
| **Out** | `1776-07-04`, `2200-01-01` | `{ "band": "out_of_range", "payload_scope": "birth_card_only" }` | Farther out — soft error envelope + birth card from month/day only |
