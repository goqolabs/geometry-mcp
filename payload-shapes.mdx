...
|---|---|---|---|
| [`get_date`](/tools/get-date) | <Badge color="green">Production</Badge> | one date | 26+ blocks across 20+ traditions — incl. `day_ruler` — <span className="field-count">~750 fields</span> |
...
<Info>
  **Production** tools (`get_date`, `get_compatibility`) are the most-tested pair. **Beta** (`get_name`) is open for builders and under active testing. Former **`get_day`** is retired — weekday ruler lives on [`get_date`](/tools/get-date#day-ruler).
</Info>
