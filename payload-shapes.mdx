...
<Info>
  **Production** tools (`get_birthdate`, `get_compatibility`) are the most-tested pair. **Beta** (`get_name`) is open for builders and under active testing. Former **`get_day`** is retired — weekday ruler lives on [`get_birthdate`](/tools/get-birthdate#day-ruler).
</Info>
