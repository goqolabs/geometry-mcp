...
**Wire names to remember:** `get_date` (single date) puts the profile at the **top level** — `cosmic` plus sibling `tarot_majors` (`.greer` / `.tarot_school`). `get_compatibility` (two dates) wraps each side as **`cosmic_card_a`** / **`cosmic_card_b`**, each holding that same profile shape. Out-of-range scope is **`cosmic_card_only`**. Every success payload also stamps **`tool` / `tool_version` / `engine_chain`**.
...
| [`get_name`](/tools/get-name) | <Badge color="green">Production</Badge> | a name | Francis Bacon Cosmic cards + Latin cipher suite + Expression / Soul Urge / Personality |

<Info>
  **Production** tools: [`get_date`](/tools/get-date), [`get_compatibility`](/tools/get-compatibility), [`get_name`](/tools/get-name). Weekday planetary ruler lives on [`get_date`](/tools/get-date#day-ruler).
</Info>
